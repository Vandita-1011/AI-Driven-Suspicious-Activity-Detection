...
import { 
  ...
  Calendar, Clock, CheckCircle2, Archive, PlusCircle 
} from 'lucide-react';
...
export default function Reports() {
  ...
  const handleGenerate = async () => {
    ...
    setGenStatus(null);
    try {
      const res = await apiService.generateReport(reportTypeToGenerate);
      setGenStatus(res);
  ...
  if (error) return <ErrorState title="Reports Error" description={error} onRetry={() => window.location.reload()} />;
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="border-b border-slate-200 pb-4">
        ...
        <p className="text-sm text-slate-500 mt-1">Generate and download AML investigation reports.</p>
      </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="space-y-4">
              ...
              <button 
                ...
                disabled={generating}
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 transition-colors disabled:opacity-50"
              >
                {generating ? "Generating..." : "Generate"}
              </button>

              {genStatus && (
                <div className="mt-3 p-3 bg-green-50 text-green-800 text-xs rounded border border-green-200 flex items-start gap-2">
        ...
        <div className="lg:col-span-3 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="p-4 border-b border-slate-200 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="relative max-w-sm w-full">
              ...
              <input 
                type="text" 
                placeholder="Search by Report ID or Customer..." 
                value={searchTerm}
                ...
                {filteredReports.length > 0 ? (
                  filteredReports.map((report) => (
                    <tr key={report.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4 font-medium text-slate-900 whitespace-nowrap">{report.id}</td>
                      <td className="px-6 py-4 text-blue-600 whitespace-nowrap cursor-pointer hover:underline">{report.alertId}</td>
                      <td className="px-6 py-4 text-slate-700 whitespace-nowrap">{report.customer}</td>
                      <td className="px-6 py-4 text-slate-700 whitespace-nowrap">{report.type}</td>
                      <td className="px-6 py-4">
                        <StatusBadge status={report.status} type="status" />
                      </td>
                      <td className="px-6 py-4 text-slate-500 whitespace-nowrap">
                        {new Date(report.generatedOn).toLocaleString(undefined, {
